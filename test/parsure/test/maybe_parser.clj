...
(defmacro p- [p s]
  `(with-parser parser-m (parse-from-string ~p ~s)))
(defn check [ret pred]
  (pred ret))
...
(with-parser parser-m
  ...
  (def p1 (domonad [x item
                    _ item
                    y item]
            (str x y)))
  (def p2 (domonad [x m-zero
                    _ item
                    y item]
            (str x y)))
  (def p3 (domonad [x (domonad [x item
                                _ item
                                y item]
                        (str x y))
                    y item]
  ...
  (deftest test-parts
    ...
    (is (check= (p- (m-bind_ spaces natural) "   123abc") 123)))
